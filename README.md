...
Приложение читает `.env` из корня проекта. В репозитории хранятся только
публичные параметры клиента:
...
только в secrets хостинга. Старый `SHIKIMORI_CLIENT_SECRET` поддерживается
временно только для локальных разработческих `.env`, но не должен попадать в
репозиторий или CI-артефакт.
