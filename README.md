...
Приложение читает `.env` из корня проекта. Файл включён в репозиторий для текущего workflow AniMix; при создании собственного форка замените значения своими OAuth-данными Shikimori:
...
`env_file` больше не используется. В CI секреты лучше передавать через защищённые переменные окружения, а не хранить в публичном репозитории.
