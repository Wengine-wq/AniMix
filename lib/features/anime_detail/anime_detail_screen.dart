...
class _AnimeDetailScreenState extends ConsumerState<AnimeDetailScreen> {
  ...
  Future<void> _fetchDataSafely() async {
    ...
    await Future.delayed(const Duration(milliseconds: 300));
    try {
      similarList = await api.getAnimes(
        limit: 10,
    ...
    try {
      ...
      if (_currentUser != null) {
        final rate = await api.getUserRate(
          widget.animeId,
        ...
        setState(() {
          ...
          if (stats != null) {
            for (var stat in stats) {
              ...
              final int value =
                  int.tryParse(stat['value']?.toString() ?? '0') ?? 0;

              // 🔥 Шикимори отдает ключи НА РУССКОМ! Ищем именно их.
              if (name == 'watching' || name == 'Смотрю') {
                _statsWatching = value;
              }
              if (name == 'planned' ||
                  name == 'В планах' ||
                  name == 'Запланировано') {
              ...
              if (name == 'completed' || name == 'Просмотрено') {
                _statsCompleted = value;
              }
            }
          }
        });
                        ...
                        child: Container(
                          ...
                          child: ClipRRect(
                            ...
                            child: SmartAnimePoster(
                              ...
                              russianTitle: anime?.russian,
                              fit: BoxFit
                                  .contain, // 🔥 Магия здесь: постер больше не режется
                            ),
                          ),
                        ),
                      ),
  ...
  Widget _buildHeaderInfo() {
    final statusText = anime?.status == 'ongoing'
        ? 'Выходит'
        ...
        Expanded(
          ...
          child: GlassButton(
            ...
            settings: const LiquidGlassSettings(
              glassColor: Color(
                0xCC8B5CF6,
              ), // Плотный фиолетовый для главной кнопки
              blur: 15,
            ),
            icon: Container(
              height: 56,
        ...
        Expanded(
          child: GlassButton(
            ...
            settings: LiquidGlassSettings(
              glassColor: currentUserStatus != null
  ...
  Widget _buildMetadataGrid() {
    ...
    final studio = anime?.studios.isNotEmpty == true
        ? anime!.studios.first
        : 'Неизвестно';
      ...
      child: Padding(
        ...
        child: Column(
          children: [
            ...
            _buildMetaRow(
              'Эпизоды',
              '${anime?.episodesAired ?? 0} / ${episodes == '0' ? '?' : episodes}',
            ),
            const Divider(color: Colors.white10, height: 24),
  ...
  Widget _buildStatsRow() {
    return Row(
      children: [
        ...
        _buildStatBox(
          'В планах',
          _statsPlanned.toString(),
          CupertinoIcons.calendar,
        ),
        const SizedBox(width: 12),
        _buildStatBox(
          'Просмотрено',
          _statsCompleted.toString(),
  ...
  Widget _buildRelatedAnimes() {
    final validRelations = relatedList
    ...
    return SizedBox(
      ...
      child: ListView.builder(
        ...
        itemBuilder: (context, index) {
          ...
          final relationText =
              item['relation_russian'] ?? item['relation'] ?? 'Связанное';
          ...
          return GestureDetector(
            onTap: () => Navigator.push(
              context,
              CupertinoPageRoute(
                builder: (_) => AnimeDetailScreen(animeId: relatedAnime.id),
              ),
            ),
            child: Container(
              width: 140,
                    ...
                    child: GlassContainer(
                      ...
                      child: ClipPath(
                        ...
                        child: SmartAnimePoster(
                          ...
                          russianTitle: relatedAnime.russian,
                        ),
                      ),
                    ),
                  ),
        ...
        itemBuilder: (context, index) {
          ...
          return GestureDetector(
            onTap: () => Navigator.push(
              context,
              CupertinoPageRoute(
                builder: (_) => AnimeDetailScreen(animeId: animeItem.id),
              ),
            ),
            child: Container(
              width: 140,
                        ...
                        children: [
                          ClipPath(
                            ...
                            child: SmartAnimePoster(
                              ...
                              russianTitle: animeItem.russian,
                            ),
                          ),
                          Container(
                            decoration: BoxDecoration(
    ...
    if (_currentUser == null) {
      showCupertinoDialog(
        ...
        builder: (ctx) => CupertinoAlertDialog(
          ...
          content: const Text(
            'Для добавления аниме в список необходимо войти в аккаунт Shikimori.',
          ),
          actions: [
                ...
                child: SafeArea(
                  child: Column(
                    ...
                    children: [
                      ...
                      Text(
                        'Оценка: ${localScore > 0 ? localScore : 'Нет'}',
                        style: TextStyle(
                      ...
                      SizedBox(
                        ...
                        child: ListView.builder(
                          ...
                          itemBuilder: (context, index) {
                            ...
                            return GestureDetector(
                              onTap: () =>
                                  setModalState(() => localScore = starValue),
                              child: Padding(
                                padding: const EdgeInsets.symmetric(
                                  horizontal: 4,
                                ),
  ...
  Widget _buildStatusOptionModal(
    String statusValue,
    String title,
    IconData icon,
    ...
    return ListTile(
      leading: Icon(
        icon,
        color: isSelected ? _accentColor : Colors.white.withValues(alpha: 0.5),
        size: 24,
      ),
      title: Text(
        title,
      ...
      trailing: isSelected
          ? Icon(CupertinoIcons.check_mark, color: _accentColor, size: 20)
          : null,
      onTap: () => onTap(statusValue),
    );
  }

  Future<void> _updateUserRate(
    String? newStatus, {
